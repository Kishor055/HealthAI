...
import { Loader2, ChevronLeft, ShieldCheck, Mail } from "lucide-react";
...
/**
 * Minimalist Email-Only Login Page.
 * Instant access protocol: No password or OTP required.
 */
export default function LoginPage() {
  ...
  const handleEmailAccess = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update/Create user record with the provided email
      ...
      toast({ title: "Access Granted", description: "Identity verified via Email. Opening clinical portal." });
        ...
        <Card className="border border-slate-100 shadow-[0_40px_100px_rgba(0,0,0,0.04)] rounded-[3rem] overflow-hidden bg-white">
          <CardContent className="p-12 md:p-16">
            <div className="text-center mb-12">
              ...
              <p className="text-lg font-medium text-slate-400">Direct email-based clinical access</p>
            </div>

            <form onSubmit={handleEmailAccess} className="space-y-8">
              ...
              <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100 flex items-start gap-4">
                ...
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-relaxed">
                  Enter your clinical email to instantly synchronize your medical history and adherence logs.
                </p>
              </div>
            </form>
            ...
            <div className="mt-16 text-center">
              <p className="text-base font-medium text-slate-400">
                New clinical user? <Link href="/signup" className="text-primary font-black uppercase tracking-widest ml-2 transition-all hover:underline">Register Now</Link>
              </p>
            </div>
          </CardContent>
        </Card>
