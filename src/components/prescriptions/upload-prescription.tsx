...
import { Camera, FileText, Loader2, PlusCircle, Sparkles, Upload, CheckCircle2, Pill, Type, History, ChevronRight, Calendar } from 'lucide-react';
               ...
               <div className="space-y-4">
                 {recentRecords.map((record) => (
                   <motion.div 
                    ...
                    animate={{ opacity: 1 }}
                    key={record.id} 
                    className="p-5 rounded-3xl border-2 bg-muted/20 hover:bg-white hover:border-primary/20 transition-all cursor-pointer group shadow-sm"
                   >
                     <div className="flex items-start justify-between">
                       ...
                       <ChevronRight className="size-4 text-muted-foreground opacity-30 group-hover:opacity-100 group-hover:translate-x-1 transition-all mt-1" />
                     </div>
                   </motion.div>
                 ))}
               </div>
